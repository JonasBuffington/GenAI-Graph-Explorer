# app\api\routers\nodes.py
from uuid import UUID
from fastapi import APIRouter, Depends, status, HTTPException, Response, Header
from app.models.graph import Node, NodeUpdate, Graph
from app.services.graph_service import GraphService
from app.db.driver import get_db_driver
from neo4j import AsyncDriver
from app.core.exceptions import NodeNotFoundException
from app.core.config import settings

router = APIRouter(
    prefix="/nodes",
    tags=["nodes"]
)

# Dependency provider for the service
def get_service(driver: AsyncDriver = Depends(get_db_driver)) -> GraphService:
    return GraphService(driver)

@router.get("/{node_id}", response_model=Node)
async def get_node(
    node_id: UUID,
    service: GraphService = Depends(get_service)
):
    """
    Retrieves a single node by its ID.
    """
    node = await service.get_node(node_id)
    if node is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Node not found")
    return node

@router.put("/{node_id}", response_model=Node)
async def update_node(
    node_id: UUID,
    node_update: NodeUpdate,
    service: GraphService = Depends(get_service)
):
    """
    Updates properties of a specific node.
    """
    updated_node = await service.update_node_properties(node_id, node_update)
    if updated_node is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Node not found")
    return updated_node

@router.delete("/{node_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_node(
    node_id: UUID,
    service: GraphService = Depends(get_service)
):
    """
    Deletes a node and all its relationships.
    """
    if not await service.delete_node(node_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Node not found")
    return Response(status_code=status.HTTP_204_NO_CONTENT)

@router.post("/{node_id}/{user_id}/expand", status_code=status.HTTP_201_CREATED, response_model=Graph)
async def expand_node(
    node_id: UUID,
    user_id: str,
    service: GraphService = Depends(get_service)
):
    try:
        created_graph = await service.expand_node(node_id, user_id)
        if not created_graph.nodes:
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="AI failed to generate valid expansion.")
        return created_graph
    except NodeNotFoundException:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Node to expand not found")