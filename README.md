...
- Health endpoints for Render (`/healthz`, requires `X-App-Revision` from clients but permits Render’s internal probe) and Redis (`/redis-health`), plus frontend UI messaging for slow cold-starts.
